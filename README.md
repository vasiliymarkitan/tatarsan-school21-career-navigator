...
- Новости: Yandex Search по `url:t.me/<канал>/*` и `@канал site:t.me` для `kazanit`, `it_tatarstan`, `innopolis_live`, `school21_kazan` (плюс HTML-превью `t.me/s/`, если оно не редиректит на лендинг). Другие сайты не добавляем. `/api/live-news` не наследует ошибки hh.ru.
